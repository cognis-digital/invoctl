# Comprehensive installer for cognis-digital/invoctl (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# invoctl is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "invoctl"
$Url  = "git+https://github.com/cognis-digital/invoctl.git"
$Git  = "https://github.com/cognis-digital/invoctl.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: invoctl"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: invoctl"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: invoctl"; exit 0 }
}
