...
[![PyPI](https://img.shields.io/pypi/v/cognis-invoctl.svg)](https://pypi.org/project/cognis-invoctl/)
[![CI](https://github.com/cognis-digital/invoctl/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/invoctl/actions)
...
INVOCTL exists for one job — cli invoicing + payment-link generator with pdf and a local ledger — and does it without a SaaS bill or heavyweight setup.
...
```bash
invoctl --version
invoctl scan .                      # scan the current project
invoctl scan . --format json
invoctl scan . --fail-on high       # non-zero exit for CI gates
invoctl mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **InvoiceNinja**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`invoctl` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
